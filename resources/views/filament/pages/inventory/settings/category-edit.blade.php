<x-filament-panels::page>
    ...
    <div class="space-y-4">
        <div>
            <h2 class="text-xl font-bold text-gray-950 dark:text-white">Edit Category</h2>
        </div>
        ...
        <form method="post" action="{{ route('inventory.categories.update', $category) }}" class="inventory-form">
            @csrf
            @method('put')
            ...
            <div class="inventory-form__grid">
                <label class="inventory-form__field">
                    ...
                    <input name="name" type="text" value="{{ old('name', $category->name) }}" class="inventory-form__control">
                ...
                <label class="inventory-form__field inventory-form__field--wide">
                    <span>Details</span>
                    <textarea name="details" rows="4" class="inventory-form__control">{{ old('details', $category->details) }}</textarea>
                    @error('details') <span class="inventory-form__error">{{ $message }}</span> @enderror
                </label>
            </div>

            <div class="inventory-form__actions">
                <button class="inventory-form__button">Update</button>
                <a href="{{ \App\Filament\Pages\Inventory\Settings\Categories::getUrl(panel: 'admin') }}" wire:navigate class="inventory-form__cancel">Cancel</a>
            </div>
        </form>
    </div>
</x-filament-panels::page>
