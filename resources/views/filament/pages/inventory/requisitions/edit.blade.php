<x-filament-panels::page>
    ...
    <style>
        ...
        .req-row-control { width: 100%; min-width: 120px; height: 38px; border: 1px solid #cbd5e1; border-radius: 8px; background: #fff; padding: 7px 10px; color: #001b33; font-size: 15px; outline: none; }
        .req-row-control--item { min-width: 220px; }
        .req-row-control--notes { min-width: 180px; }
        ...
        .dark .req-form-card, .dark .req-form-control, .dark .req-form-cancel, .dark .req-items, .dark .req-row-control { border-color: #334155; background: #111827; color: #fff; }
    ...
    <div class="req-form-page space-y-6">
        <div>
            <h2 class="req-form-title">Edit Requisition</h2>
            <p class="req-form-subtitle">Update purchase requisition details</p>
        </div>

        <form wire:submit="updateRequisition" class="req-form-card">
                        ...
                        <table class="req-items-table">
                            ...
                            <tbody>
                                @foreach ($selectedItems as $selectedItem)
                                    <tr wire:key="requisition-edit-selected-{{ $loop->index }}">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            <select
                                                wire:model="selectedItems.{{ $loop->index }}.inventory_item_id"
                                                wire:change="refreshSelectedItemRow({{ $loop->index }})"
                                                class="req-row-control req-row-control--item"
                                        ...
                                        <td>
                                            <select
                                                wire:model="selectedItems.{{ $loop->index }}.inventory_unit_id"
                                                wire:change="refreshSelectedUnitRow({{ $loop->index }})"
                                                class="req-row-control"
                                                aria-label="Unit {{ $loop->iteration }}"
                                            >
                                                @foreach ($units as $unit)
                                                    <option value="{{ $unit->id }}">{{ $unit->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('selectedItems.' . $loop->index . '.inventory_unit_id') <span class="req-form-error">{{ $message }}</span> @enderror
                                        </td>
                                        <td>
                                            <input
                                                wire:model="selectedItems.{{ $loop->index }}.quantity"
                                                type="number"
                                                min="1"
                                                step="0.01"
                                                class="req-row-control"
                                                aria-label="Quantity {{ $loop->iteration }}"
                                            >
                                            @error('selectedItems.' . $loop->index . '.quantity') <span class="req-form-error">{{ $message }}</span> @enderror
                                        </td>
                                        <td>
                                            <input
                                                wire:model="selectedItems.{{ $loop->index }}.notes"
                                                type="text"
                                                class="req-row-control req-row-control--notes"
                                                aria-label="Notes {{ $loop->iteration }}"
                                            >
                                            @error('selectedItems.' . $loop->index . '.notes') <span class="req-form-error">{{ $message }}</span> @enderror
                                        </td>
                                        <td>
            ...
            <div class="req-form-actions">
                <button type="submit" class="req-form-submit" @disabled(! count($selectedItems)) wire:loading.attr="disabled" wire:target="updateRequisition">
                    Update Requisition
                </button>
